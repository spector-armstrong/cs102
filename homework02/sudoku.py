...
def group(values: tp.List[T], n: int) -> tp.List[tp.List[T]]:
    ...
    >>> group([1,2,3,4], 2)     # каждая строка это список в большом списке поэтому используется "генератор"
    [[1, 2], [3, 4]]
...
def get_row(grid: tp.List[tp.List[str]], pos: tp.Tuple[int, int]) -> tp.List[str]:
    ...
    >>> get_row([['1', '2', '.'], ['4', '5', '6'], ['7', '8', '9']], (0, 0))    # функция читает только ряд поэтому запрашивается в еортеже только номер ряда под индексом 0 и 0 колонка
    ['1', '2', '.']
